/*!
 * Atma File System Module v0.2.34
...
(function(root, factory){
	...
	// source /src/directory.es6
	"use strict";
	
	(function () {
	
		var Directory = io.Directory = Class({
			Construct: function Construct(directory) {
				...
				if (typeof directory === "string" && directory[directory.length - 1] !== "/") {
					logger.warn("@ directory path should end with slash. Adding...", directory);
	
					if (/\.\w+$/.test(directory) === false) directory = directory + "/";
				}
			...
			readFiles: function readFiles(pattern, exclude) {
	
				var patterns = glob_parsePatterns(pattern),
				    excludes = glob_parsePatterns(exclude),
				    that = this;
	
				this.files = dir_files(this.uri.toLocalDir(), patterns, excludes).map(function (x) {
					return new io.File(that.uri.combine(x));
				});
	
				return this;
			},
			read: function read(pattern, exclude) {
	
				var patterns = glob_parsePatterns(pattern),
				    excludes = glob_parsePatterns(exclude),
				    that = this;
	
				return dir_files(this.uri.toLocalDir(), patterns, excludes, { directories: true }).map(function (x) {
					var path = that.uri.combine(x);
					if (x[x.length - 1] === "/") return new io.Directory(path);
	
					return new io.File(path);
				});
			},
	
			readFilesAsync: function readFilesAsync(pattern, exclude) {
				var patterns = glob_parsePatterns(pattern),
				    excludes = glob_parsePatterns(exclude);
			...
			readAsync: function readAsync(pattern, exclude) {
				var patterns = glob_parsePatterns(pattern),
				    excludes = glob_parsePatterns(exclude);
	
				return dfr_factory(this, function (dfr, dir, path) {
					dir_filesAsync(path, patterns, exclude, { directories: true }, function (error, files) {
						if (error) {
							dfr.reject(error);
							return;
						}
	   ...
	   */
			copyTo: function copyTo(target, options) {
				var dfr = new Class.Deferred();
				if (Array.isArray(this.files) === false) this.readFiles();
	   ...
	   */
			copyToAsync: function copyToAsync(target, options) {
				var dfr = new Class.Deferred();
				if (Array.isArray(this.files) === false) {
	
					var dir = this;
					this.readFilesAsync().done(function () {
						dir.copyToAsync(target, options).done(dfr.resolveDelegate()).fail(dfr.rejectDelegate());
				...
				await.done(dfr.resolveDelegate()).fail(dfr.rejectDelegate());
				...
				function copy(i, done) {
					...
					if (options.verbose !== true && io.File.exists(to)) {
						...
						cli_prompt(message, function (confirm) {
							...
							file.copyToAsync(to).done(onComplete).fail(done);
						});
						return;
					}
					file.copyToAsync(to).done(onComplete).fail(done);
			...
			renameAsync: function renameAsync(name) {
				return dfr_factory(this, function (dfr, dir, path) {
					if (!name) {
			...
			watch: function watch(callback) {
				io.watcher.watch(this.uri.toLocalFile(), callback);
			},
			unwatch: function unwatch(callback) {
				// - callback: if undefined remove all listeners
				io.watcher.unwatch(this.uri.toLocalFile(), callback);
			},
	...
	//# sourceMappingURL=directory.es6.map
	// end:source /src/directory.es6
...
(function () {
	...
	var Functions = {
		version: (function (_version) {
			var _versionWrapper = function version() {
				return _version.apply(this, arguments);
			};

			_versionWrapper.toString = function () {
				return _version.toString();
			};

			return _versionWrapper;
		})(function () {
