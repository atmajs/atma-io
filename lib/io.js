...
(function(root, factory){
	...
	(function () {
		...
		function Importer(file) {
			...
			if (rgx_version.test(code)) code = processVersion(code);
		...
		function process(currentUri, code) {
			...
			return code.replace(rgx_importStatement, function (full, isString, match1, full2, match2) {
				...
				if (path.indexOf("*") !== -1) {
					files = new io.Directory(glob_getStrictPath(path)).readFiles(glob_getRelativePath(path)).files;
				}
				...
				var indent = full.substring(0, full.indexOf("//")),
					...
					var msg = "File Import %1 into %2".green.format(uri.file, baseUri.file);
				...
				return full.replace("import", "source") + io.env.newLine + content + io.env.newLine + full.replace("import", "end:source");
			});
		}
		...
		function get_fileContent(file, indent, insertFileName) {
			...
			if (insertFileName) {
				content = indent + "// source " + file.uri.file + io.env.newLine + content;
			}
			return content;
		}
