...
function parseStackEntry(line) {
    ...
    if (currentFile == null) currentFile = stackEntry.file;

    return stackEntry;
}

export function stack_formatCaller(format = 'P:L') {
    ...
    while (++i < imax) {
        ...
        if (entry == null || currentFile == null) continue;

        if (entry.file !== currentFile) break;
    }

    if (entry == null || i === imax) return "";
