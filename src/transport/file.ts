...
export const FileTransport: ITransport = {
    ...
    copyAsync(from, to, cb) {
        FileTransport.existsAsync(from, prepairFn);
    ...
    remove(path) {
        if (FileTransport.exists(path) === false) return true;
    ...
    rename(path, filename) {
        if (FileTransport.exists(path) === false) {
